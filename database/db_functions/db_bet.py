...
class BetRepository:
    ...
    async def _log_bet_event(self, user_id: int, outcome_id: int, amount: int, option: int, event_type: str, server_id: int):
    ...
    async def withdraw_bet_for_user(self, bet_theme: str, user_id: int, server_id, time_now: int) -> BetWithdrawResult:
        try:
            ...
            await self.session.execute(update(BetEvents).where(
            ...
            await self.session.execute(update(Balance)
                .where(Balance.id == internal_id)
                .values(balance=Balance.balance + participation.money)
            )
            await self.session.execute(delete(BetParticipation).where(
    ...
    async def process_place_bet(self, user_discord_id: int, bet_theme: str, choice_text: str, amount: int, server_id: int) -> BetPlaceResult:
        try:
