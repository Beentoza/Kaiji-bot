...
class ItemRepository:
    ...
    async def _item_type_id(self, item_name: str) -> int | None:
    ...
    async def add_item(self, item_name, in_casino, role, emoji, on_author, duration):
        try:
            self.session.add(ItemTypeInfo(
                item_name=item_name,
                in_casino=in_casino,
                role=role,
                emoji=emoji,
    ...
    async def change_item(self, item_name, in_casino=None, role=None, emoji=None, on_author=None, duration=None):
    ...
    async def item_name_exists(self, item_name) -> bool:
        """True if a catalog item type with this name already exists."""
        return await self._item_type_id(item_name) is not None


    async def get_catalog_items(self) -> list:
    ...
    async def get_item_settings(self, item_name):
    ...
    async def add_item_to_user(self, user_id: int, item: str, amount: int = 1):
        ...
        internal_id = await self.session.scalar(
            select(User.id).where(User.discord_id == user_id)
        )

        item_id = await self._item_type_id(item)
    ...
    async def get_user_inventory(self, discord_id: int):
    ...
    async def consume_item(self, discord_id: int, item) -> bool:
        internal_user_id = await self.session.scalar(
            select(User.id).where(User.discord_id == discord_id))
    ...
    async def get_winning_items(self) -> dict:
        """Return {emoji: item_name} for casino item types that have an emoji set.
        The caller owns the self.session. Keys double as the slot-machine symbols."""
    ...
    async def get_item_role(self, item_name: str) -> int | None:
        return await self.session.scalar(
    ...
    async def transfer_item(self, from_discord_id: int, to_discord_id: int, item_name: str, amount: int = 1) -> bool:
        from_internal_id = (await self.session.execute(
            select(User.id).where(User.discord_id == from_discord_id)
        )).scalar_one()
        to_internal_id = (await self.session.execute(
            select(User.id).where(User.discord_id == to_discord_id)
        )).scalar_one()
        item_id = await self._item_type_id(item_name)
        if item_id is None:
            return False

        await self.session.execute(select(UserItems) # blocking both ID so we can't get deadlock
                .where(
        ...
        if (await self.session.execute(stmt)).rowcount == 0:
            return False
