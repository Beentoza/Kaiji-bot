...
async def log_outcome_event(session, user_id: int, outcome_id: int, outcome_name: str, event_type: str, server_id: int) -> None:
...
async def add_new_bet(session, theme: str, options: list, end_timestamp: int, message_id: int, channel_id: int, server_id: int, user_id: int) -> int | None:
    ...
    bet_id = (await session.execute(stmt)).scalar_one_or_none()

    if bet_id is None:
        logger.info(f"Outcome {theme} already exists on server {server_id}, skipping")
        return None

    await log_outcome_event(session, user_id=user_id, outcome_id=bet_id, outcome_name=theme,
                            event_type='created', server_id=server_id)
    return bet_id


async def get_bet_with_same_name(session, outcome: str, server_id) -> bool:
