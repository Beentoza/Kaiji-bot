...
async def add_new_user(session, user_id, status: int = 1, balance: int = 100, luck_factor: float = 0) -> bool:
    ...
    internal_id = (await session.execute(stmt)).scalar_one_or_none()
...
async def get_user_status(session, user_id: int):
    try:
        stmt = (
...
async def change_user_status(session, user_id: int, new_status: int):
    try:
...
async def get_user_balance(session, user_id: int):
    try:
...
async def set_user_balance(session, user_id: int, balance_value: int) -> bool:
    try:
        stmt = select(Balance).join(User).where(User.discord_id == user_id)
        result = await session.execute(stmt)
        balance_obj = result.scalar_one_or_none()
...
async def set_user_status(session, user_id: int, status: int):
    try:
        stmt = select(Status).join(User).where(User.discord_id == user_id)
        result = await session.execute(stmt)
...
async def get_user_chances_data(session, user_id: int):
...
async def get_overall_user_info(session, user_id: int):
    try:
        stmt = (
            select(
...
async def get_week_balance_history(session, user_id: int):
