...
class EconomyRepository:
    ...
    async def get_jackpot_info(self) -> str | None:
        try:
            result = await self.session.execute(select(Jackpot.money).limit(1))
    ...
    async def add_to_jackpot(self, amount: int):
        try:
            await self.session.execute(update(Jackpot).values(money=Jackpot.money + amount))
        except Exception as e:
            logger.error(f"Error: {e}")
            raise
    ...
    async def get_lottery_info(self, user_id: int):
        try:
    ...
    async def get_timestamp_balance_status(self, user_id: int, timestamp_column_name: str):
        try:
    ...
    async def get_balance_status_luckfactor(self, user_id: int):
        try:
            stmt = (
    ...
    async def get_pickup_change_info(self, user_id: int):
        try:
            stmt = (
                select(
    ...
    async def update_personal_pickupchange_info(self, user_id, timestamp, cooldown):
        try:
    ...
    async def update_global_pickupchange_info(self, timestamp, cooldown):
        try:
            await self.session.execute(
                update(WorldState)
                .where(WorldState.id == 1)
